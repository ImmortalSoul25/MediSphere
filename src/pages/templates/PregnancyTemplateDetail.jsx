...
export default function PregnancyTemplateDetail() {
  ...
  useEffect(() => {
    if (pregnancy) {
      const found = pregnancy.find((t) => String(t.week) === String(week));
      if (found) {
  ...
  if (error || (!loading && !template)) {
    return (
      <div className="p-6">
        <button onClick={() => navigate("/message-templates")} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-800 mb-6 transition-colors">
  ...
  const handleSendForApproval = async () => {
    ...
    try {
      ...
      await updatePregnancyTemplate(template.week, updatedData);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Pregnancy Week {template.week}</p>
          </div>
          {!isEditing && (
            <button 
              onClick={handleEdit}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Week Number</label>
              <div className="text-sm font-medium text-slate-800">{template.week}</div>
            </div>
          ...
          <div className="pt-2">
             <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">YouTube Link</label>
             {isEditing ? (
               <input 
                 type="url"
                 value={draft.yt_link || ""}
               ...
               <div className="text-sm font-medium text-indigo-600 truncate hover:underline">
                 {template.yt_link ? <a href={template.yt_link} target="_blank" rel="noopener noreferrer">{template.yt_link}</a> : <span className="text-slate-400 no-underline cursor-default">None</span>}
               </div>
             )}
          </div>
        </div>
